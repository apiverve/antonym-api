...
class AntonymValidationException implements Exception {
  final List<String> errors;

  AntonymValidationException(this.errors);

  @override
  String toString() => 'AntonymValidationException: ${errors.join("; ")}';
}
...
/// Antonym Finder API client.
///
/// For more information, visit: https://antonym.apiverve.com?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [word] (required) - The word to get the antonyms for
class AntonymClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'word': ValidationRule(type: 'string', required: true),
  };

  AntonymClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/antonym',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [AntonymValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw AntonymValidationException(errors);
    }
  }

  /// Execute a request to the Antonym Finder API.
  ///
  /// Parameters are validated before sending the request.
  Future<AntonymResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw AntonymException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return AntonymResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw AntonymException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw AntonymException('Resource not found');
      } else {
        throw AntonymException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is AntonymException) rethrow;
      throw AntonymException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Antonym Finder API client.
class AntonymException implements Exception {
  final String message;

  AntonymException(this.message);

  @override
  String toString() => 'AntonymException: $message';
}
